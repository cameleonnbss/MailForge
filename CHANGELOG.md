...
- Chaine de compilation adaptative (`scripts/build.js`) : detection unique de la possibilite de creer un lien symbolique, puis chaine standard d'electron-builder ou chemin de remplacement (`--dir` + rcedit + cibles `--prepackaged`). Contourne l'impossibilite d'extraire l'outil de signature d'electron-builder sur une machine Windows sans mode developpeur, sans droits administrateur ni certificat ; l'executable porte malgre tout l'icone et les metadonnees `ProductName`, `FileVersion`, `CompanyName`, `LegalCopyright`.
- Integration continue : verification (syntaxe + `npm run smoke`) a chaque push, construction et publication automatiques des trois artefacts sur un tag, plus un declenchement manuel pour reconstruire les binaires a la demande. Le binaire package est lui-meme auto-teste avant publication.
